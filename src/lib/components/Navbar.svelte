<script lang="ts">
	...
	afterNavigate(() => {
		...
		// Close any open dropdowns or mobile drawer on route change
		ddClose('projects');
		ddClose('explore');
		isOpen = false;
		mobileExpanded = null;
	...
	onMount(() => {
		...
		function handleGlobalClick(e: MouseEvent) {
			...
			if (capsuleEl && !capsuleEl.contains(target)) {
				ddClose('projects');
				ddClose('explore');
			}
		}
...
<header
	class="fixed z-100 inset-x-0 top-3 sm:top-4 md:top-6 px-3 sm:px-4 md:px-6 flex justify-center pointer-events-none transition-all duration-300"
...
>
	<div
		...
		role="presentation"
		class="w-full max-w-[1320px] h-14 sm:h-16 md:h-18 rounded-3xl sm:rounded-4xl flex items-center justify-between px-4 sm:px-6 md:px-8 border pointer-events-auto transition-[background-color,box-shadow] duration-500 ease-out relative liquid-glass {capsuleClasses}"
	>
		...
		<div class="flex items-center gap-5 shrink-0 relative z-10">
			...
			<a href="/" class="flex flex-col leading-none group">
				<span class="text-[0.92rem] sm:text-[1.02rem] md:text-[1.05rem] font-black tracking-[-0.04em] transition-opacity group-hover:opacity-75 {currentTheme === 'dark' ? 'text-white' : 'text-[#1B1916]'}">
					HITANSHU SAHU
				</span>
				<span class="text-[8px] sm:text-[8.5px] font-mono tracking-[0.22em] uppercase mt-[3px] {currentTheme === 'dark' ? 'text-white/30' : 'text-black/35'}">
					Product Designer
				</span>
			</a>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 mt-6 border-t border-white/10 w-full text-sm text-neutral-400">
			...
			<div class="flex items-center gap-6 md:justify-end flex-wrap">
				<a href="https://www.linkedin.com/in/phantom-cluster/" target="_blank" rel="noopener noreferrer" class="hover:text-primary text-white transition-all flex items-center gap-2 group text-xs font-bold uppercase tracking-wider">
					<!-- @ts-ignore -->
					<ion-icon name="logo-linkedin" aria-hidden="true" style="font-size: 18px; color: rgba(163,163,163,1); transition: color 0.2s;" class="group-hover:text-primary"></ion-icon>
					<span>LinkedIn</span>
				</a>
				...
				<a href="https://behance.net/phantomcluster" target="_blank" rel="noopener noreferrer" class="hover:text-primary text-white transition-all flex items-center gap-2 group text-xs font-bold uppercase tracking-wider">
					<!-- @ts-ignore -->
					<ion-icon name="logo-behance" aria-hidden="true" style="font-size: 18px; color: rgba(163,163,163,1); transition: color 0.2s;" class="group-hover:text-primary"></ion-icon>
					<span>Behance</span>
				</a>
			</div>
		</div>
