<script lang="ts">
	...
	function handleMouseMove(e: MouseEvent) {
		...
		capsuleEl.style.setProperty('--mouse-x', `${x}px`);
		capsuleEl.style.setProperty('--mouse-y', `${y}px`);
...
<!-- FLOATING LIQUID GLASS NAVIGATION BAR (Apple Developer Guidelines) -->
<header
	class="fixed z-50 inset-x-0 top-6 px-4 md:px-6 flex justify-center pointer-events-none transition-all duration-300"
	aria-label="Main navigation"
>
	<!-- Continuous unified Apple liquid glass capsule. 
	     Frosted blur and saturated opacity completely masks scrolling text underneath. -->
	<div 
		bind:this={capsuleEl}
	...
	>
		...
		<a
			href="/"
			class="relative z-10 text-lg md:text-xl font-extrabold tracking-tight text-neutral-950 dark:text-white hover:opacity-90 transition-opacity flex items-center gap-1"
		>
		...
		<div class="flex items-center gap-3 relative z-10">
			...
			<a
				href="/#contact"
				class="hidden md:inline-flex bg-neutral-950 hover:bg-neutral-900 dark:bg-white dark:hover:bg-neutral-100 text-white dark:text-neutral-950 px-5 py-2.5 rounded-full text-xs font-extrabold uppercase tracking-wider items-center gap-2 transition-all active:scale-95 shadow-md shadow-black/10"
			>
				Contact Us
			</a>
			...
			<button
				...
				class="inline-flex md:hidden bg-neutral-950 hover:bg-neutral-900 dark:bg-white dark:hover:bg-neutral-100 text-white dark:text-neutral-950 px-5 py-2.5 rounded-full text-xs font-extrabold uppercase tracking-wider items-center gap-2.5 transition-all active:scale-95 shadow-md shadow-black/10"
				aria-label="Toggle navigation menu"
			>
				<span>Menu</span>
				{#if isOpen}
					<X class="size-3.5 text-primary animate-spin-once" />
				{:else}
					<Menu class="size-3.5 text-white dark:text-neutral-950" />
				{/if}
			</button>
		</div>
	</div>
...
{#if isOpen}
	...
	>
		...
		<!-- Bottom Details: Monochrome SVG Social Icons (LinkedIn, Dribbble, Behance) -->
		<div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-white/10 w-full text-sm text-neutral-400">
			...
			<!-- Social monochrome SVG icons at exact 16px (size-4) next to text links -->
